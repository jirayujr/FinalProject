...
import 'package:app_repair/bodys/show_detail_massage.dart';
...
class _ShowMessageState extends State<ShowMessage> {
  ...
  @override
  void initState() {
    print("in_showMessage");
    // TODO: implement initState
    super.initState();
    readMybooking();
  }

  showTypeUser(String id){
...
/*
  ...
  SetTypeUser(String type){
    typeUser = type;
    print("typeUserShowMessage :: $typeUser");
  }
  ...
  Future<Null> readMybooking() async {
    ...
    String ans = await obj.showTypeUser(id!);
    print("ans :: $ans");
    SetTypeUser(ans);

    if(ans == "technician"){
      print("if ans == technician");
      ...
      await Dio().get(showNewJob).then((value) {
        print("==getTableMatchJobWhereUser==");
        print('==>$value');
      ...
      String showNewJob = '${MyConstant.domain}/repairer_app/getTableMatchJobWhereUserIdCustomer.php?isAdd=true&id=$id';
      await Dio().get(showNewJob).then((value) {
        print("==getTableMatchJobWhereUser==");
        print('==>$value');
        if (value.toString() == 'null') {
  ...
  /*
  firstnameWithIdRepairer(String idRepairer) async {
    backend obj = new backend();
    String firstname = await obj.showFirstName(idRepairer);
    return firstname;
  }
  */
  typeWithIdJob(String id_job){
    String typeRepairerAns = "nondateTypeRepairer";
    for(int i = 0;i<GiveFix2Models.length;i++){
      if(GiveFix2Models[i].id_job == id_job){
        typeRepairerAns = GiveFix2Models[i].typeRepairer;
        break;
      }
    }
    return typeRepairerAns;
  }
                      ...
                      children: [
                        Row(
                          ...
                          children: [
                            ...
                            Text(
                              //tableMatchJobModels[index].id_customer +
                              typeWithIdJob(tableMatchJobModels[index].id_job),
                                  //+
                                 // ' '+lastnameWithIdCustomer(tableMatchJobModels[index].id_repairer) ,
                              //NumberMatchStatus[index].toString() +
                              //    UserModel_Models[NumberMatchStatus[index]].firstname.toString()+' '+UserModel_Models[NumberMatchStatus[index]].lastname.toString(),
                              //firstnameArrText[numberMatch[index]]+' '+lastnameArrText[numberMatch[index]],
                              style: TextStyle(
                            ...
                            IconButton(
                              onPressed: () {
                                ...
                                Navigator.push(
                                    context,
                                    MaterialPageRoute(
                                      builder: (context) => ShowDetailMessage(tableMatchJobModels[index].id_customer,typeUser,tableMatchJobModels[index].id_repairer),
                                    ));
                              },
                              icon:
                        ...
                        Text(
                          "idJob:: "+tableMatchJobModels[index].id_job,
                              //+tableMatchJobModels[index].orderMatch,
